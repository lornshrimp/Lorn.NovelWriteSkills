...
3. 每个平台写入后必须显式运行 `scripts/platform_validate.ps1` 做正文与 `## 作者有话说` 联合门禁；中文平台 gate 至少包含 `minCJK=3500, maxCJK=6500, requireAfterword=true, minAfterwordCJK=200, maxAfterwordCJK=300`，英文平台 gate 至少包含 `minLen=6500, requireAfterword=true, minAfterwordWords=100, maxAfterwordWords=150`；若只补测作者有话说，中文用 `scripts/count-afterword.ps1 -MinCJK 200 -MaxCJK 300`，英文用 `scripts/count-afterword-words.ps1 -MinWords 100 -MaxWords 150`
