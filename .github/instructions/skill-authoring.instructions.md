...
在编写、扩写、改写或审阅题材 Skill 的整个过程中，必须持续重复做这项判断，而不是只在开始时判断一次。

- 只要识别出某段规则、流程、模板、检查项、边界或说明其实**不专属于该题材**，就必须将其提取、沉淀或回收进对应能力的通用 Skill
- 这条要求**不以本次任务是否显式要求修改通用 Skill 为前提**；即使用户只提到题材 Skill，也仍然应同步考虑并推动通用沉淀
- 不得因为“这次只是顺手改题材 Skill”“先放在题材 Skill 里以后再说”“通用 Skill 本次未点名修改”而跳过沉淀
- 若对应能力尚无通用 Skill，应优先补建该通用 Skill，再让题材 Skill 回到题材包装层 / 路由层职责
...
- 在修改题材 Skill 时，凡是新写入、补写、扩写或重写出的通用内容，也应立即回收到对应通用 Skill，而不是只在题材 Skill 内部留存
...
- 对题材 Skill 的任何改造，都必须顺带检查新增内容里是否混入了题材无关的共性本体；一旦混入，就必须同步抽离到通用 Skill 中沉淀
...
- 在编写或修改题材 Skill 的过程中，是否持续检查了新增内容里哪些其实是题材无关的通用内容
- 若发现题材无关的通用内容，是否无论本次任务有没有点名，都已经同步提取并沉淀到对应通用 Skill
...
- 改题材 Skill 时，把“持续识别并回收题材无关内容到通用 Skill”视为默认动作，而不是额外可选动作
