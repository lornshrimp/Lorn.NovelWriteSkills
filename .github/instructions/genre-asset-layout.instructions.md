...
- 在编写、扩写、改写或审阅题材 Skill 的整个过程中，必须持续检查新增内容里哪些其实不专属于该题材；一旦识别出题材无关的通用内容，就应无论本次任务是否点名通用 Skill，都同步提取并沉淀到对应通用 Skill
- 若对应通用 Skill 尚未存在，应优先补建通用 Skill，再让题材 Skill 回到包装层、路由层与题材边界职责，而不是把题材 Skill 长成通用本体替身
...
- 即使本次任务入口是题材目录内文件，只要处理中识别出题材无关的共性规则、模板、检查项或流程，也应同步上收到 `通用skills/`，不要因为当前落点在题材目录就继续保留在题材层
